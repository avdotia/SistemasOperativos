...
/*clase creada para la organización, ejecución y control de los hilos del sistema. 
...
la lista de preparados. La política de planificación es FIFO: el hilo en ejecución 
sólo abandona la CPU cuando la entrega (yield) o cuando se bloquea (con un semáforo, 
cerrojo o variable condición). Utiliza la función switch  para el cambio de contexto.*/
...
//Cambiamos la politica de FCFS (orden de llegada) por una política de método con prioridades
...
/*El 'SortedInsert' lo que hace es insertar en una lista el elemento dado según la prioridad.
Si la lista está vacía  es el único, si no recorre la lista y lo inserta donde corresponda*/
...
/* Elimina el primer elemento de la lista ordenada y
 ...
 Establece un puntero a la ubicación en la que se almacena la prioridad del
