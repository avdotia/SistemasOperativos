...
/*Para definir hilos concurrentes. Ofrece primitivas básicas (Fork, Sleep, Yield y Finish). 
Para saber qué código ha de ejecutar el hilo, al invocar a Fork() le pasamos como parámetro 
un puntero a función. Los hilos se apoyan en la clase Scheduler para la planificación. */
...
//----------------------------------------------------------------------
// Cambios constructor nuevo
//----------------------------------------------------------------------
//obtenemos la prioridad del hilo con esta función
    int 
	Thread::prio()
	{
		return prioridad;
	}
//el constructor nuevo, con el campo prioridad del hilo que hemos obtenido de la
//funcion prio
	Thread::Thread(const char* Nom_Hilo, int p)
	{
    		name = Nom_Hilo;
   		stackTop = NULL;
    		stack = NULL;
...
{
    Thread *nextThread;
//si el actual es el que esta en ejecucion

    ASSERT(this == currentThread);
// comprueba que las interrupciones esta desactivadas

    ASSERT(interrupt->getLevel() == IntOff);
    
    DEBUG('t', "Sleeping thread \"%s\"\n", getName());
// el cambiamos el status a Bloqueado

    status = BLOCKED;
// si hay elementos en cola, lo preparamos para ejecutar(el siguiente)
